...
function ExternalLogger() {
   ...
   * @private
   */
  let prepareLogToConsole = function (method) {
      let console = window.console,
        ...
        {
          let isPhantomJS = new BrowserDetect().browser != "PhantomJS";
...
function enhanceLog($log) {
  let separator = "::",
    ...
    colorify = function (message, colorCSS) {
      let isChrome = detector.browser == "Chrome",
...
function makeTryCatch(notifyFn, scope) {
   ...
   */
  let reportError = function (reason) {
      if (notifyFn != null) {
        let error = (reason && reason.stack) ? reason : null,
    ...
    tryCatch = function (actionFn, scope, args) {
      try {
        ...
        let result = angular.isFunction(actionFn) ? actionFn.apply(scope, args || []) : String(actionFn),
...
function DateTime() {
   ...
   */
  let buildTimeString = function (date, format) {
